        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden items-center gap-7 lg:flex">
            {HOME_NAV_ITEMS.map((item) => (
              <Link key={item.href} to={item.href} className="wc-body-sm text-[0.95rem] font-[600] transition-colors hover:text-[var(--wc-blue)]">
                {item.label}
              </Link>
            ))}
            <Link to="/#contact" className="wc-btn-primary px-5 py-3">
              Bezpłatna wycena
            </Link>
          </div>
        ...
        {isMenuOpen && (
          <div className="wc-surface-card mt-4 p-4 lg:hidden">
            <div className="flex flex-col gap-2">
              ...
              <Link to="/#contact" className="wc-btn-primary mt-2 justify-center" onClick={() => setIsMenuOpen(false)}>
                Bezpłatna wycena
              </Link>
            </div>
          </div>
